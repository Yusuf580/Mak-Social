...
const App: React.FC = () => {
  ...
  const [activeSector, setActiveSector] = useState<College | "Global">(
    "Global",
  );
  ...
  useEffect(() => {
    ...
    if (savedUserId) {
      ...
      if (user) {
        ...
        setIsLoggedIn(true);
        setuserRole(
          user.email?.endsWith("@admin.mak.ac.ug") ? "admin" : "student",
        );
        setView("home");
      }
    }
      ...
      case "profile":
        return (
          <Profile
            ...
            onNavigateToProfile={(id) => setSelectedUserId(id)}
            onMessageUser={() => setView("chats")}
            onUpdateCurrentUser={(u) => setCurrentUser(u)}
          />
        );
      case "calendar":
        return <CalendarView isAdmin={userRole === "admin"} />;
      case "admin-calendar":
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--bg-primary)] text-[var(--text-primary)] font-sans">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header
          ref={headerRef}
          className="sticky top-0 z-[80] bg-[var(--bg-primary)]/80 backdrop-blur-md border-b border-[var(--border-color)] px-4 sm:px-6 py-4 flex items-center justify-between shadow-sm"
        >
          <div className="flex items-center gap-2 sm:gap-3 overflow-visible">
            <button
              onClick={() => setIsSidebarOpen(true)}
              className="p-2 text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] rounded-full lg:hidden shrink-0"
            >
              <Menu size={22} />
            </button>
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl transition-all group max-w-[150px] sm:max-w-none"
              >
                <div className="shrink-0 text-[var(--brand-color)]">
                  {activeSector === "Global" ? (
                    <Globe size={16} />
                  ) : (
                    <LayoutGrid size={16} />
                ...
                <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-[var(--brand-color)] truncate">
                  {activeSector} HUB
                </span>
                <ChevronDown
                  size={12}
                  className={`text-slate-500 transition-transform shrink-0 ${isSectorDropdownOpen ? "rotate-180" : ""}`}
                />
              </button>
              {isSectorDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-64 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl shadow-2xl z-[500] p-3 animate-in slide-in-from-top-2">
                  <button
                    onClick={() => {
                      setActiveSector("Global");
                      setIsSectorDropdownOpen(false);
                      setView("home");
                    }}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl mb-2 ${activeSector === "Global" ? "bg-[var(--brand-color)] text-white" : "hover:bg-[var(--bg-secondary)] text-slate-400"}`}
                  >
                    <Globe size={16} />{" "}
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {[
                      ...
                      <button
                        key={c}
                        onClick={() => {
                          setActiveSector(c as College);
                          setIsSectorDropdownOpen(false);
                          setView("home");
                        }}
                        className={`flex items-center justify-center py-2.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${activeSector === c ? "bg-[var(--brand-color)] text-white" : "hover:bg-[var(--bg-secondary)] text-slate-400"}`}
                      >
                        {c}
                      </button>
                    ))}
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0 relative ml-2">
            <div className="relative shrink-0">
              ...
              >
                ...
                {unreadMsgs > 0 && (
                  <span className="absolute top-1 right-1 w-4 h-4 bg-[var(--brand-color)] text-white text-[8px] font-black rounded-full flex items-center justify-center border-2 border-white shadow-sm">
                    {unreadMsgs}
                  </span>
                )}
              </button>
            ...
            <div className="relative shrink-0">
              ...
              {isNotifOpen && (
                <NotificationDropdown
                  ...
                  onViewAll={() => handleSetView("notifications")}
                />
              )}
            </div>
